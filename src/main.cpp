...
#define FREECAM_SPEED 1.0f //velocidade da camera, em m/s
...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 600, "teste", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //computacao dos movimentos da camera
        glm::vec4 side = crossproduct(cam.getViewVec(),cam.getUpVec());
        if(iskeydown_w){cam._move(timeDelta * FREECAM_SPEED * normalize(cam.getViewVec()));}
        if(iskeydown_s){cam._move(timeDelta * -FREECAM_SPEED * normalize(cam.getViewVec()));}
        if(iskeydown_a){cam._move(timeDelta * -FREECAM_SPEED * normalize(side));}
        if(iskeydown_d){cam._move(timeDelta * FREECAM_SPEED * normalize(side));}
