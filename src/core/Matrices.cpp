...
float angleAroundY(glm::vec4 v) {
    ...
    float magnitudeRef = glm::length(ref);
    float magnitudeV = glm::length(v);
...
{
    glm::vec4 _front = glm::normalize(target - position);
    glm::vec4 right = glm::normalize(crossproduct(up, _front));
    up = crossproduct(_front, right);
    return glm::mat4(right.x, up.x, _front.x, position.x,
                     right.y, up.y, _front.y, position.y,
                     right.z, up.z, _front.z, position.z,
                     0, 0, 0, 1);
}
